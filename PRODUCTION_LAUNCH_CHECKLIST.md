...
- [ ] Edge functions deployed with updated secrets: `supabase functions deploy chapa-service chapa-webhook admin_actions freshness_cron telegram-webhook` (freshness_cron + telegram-webhook with `--no-verify-jwt`; set `CRON_SECRET`, `RESEND_API_KEY`, `APP_URL`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `TELEGRAM_CHAT_ID` via `supabase secrets set`)
- [ ] Migrations applied to the production DB (`supabase db push`) — 34 migrations. Latest: `20260920010000_replace_dead_unsplash_images.sql` (swaps the 33 dead seeded Unsplash URLs for local branded SVGs); `20260920000000_notifications_in_app.sql` (in-app notifications: table + own/admin RLS + realtime + mark-read RPC); `20260919010000_freshness_automation.sql` (pg_cron flagging + alert bookkeeping); `20260919000000_fix_entitlement_policy_function_privilege.sql` (role-split policies, 42501 regression caught by `npm run probe:rls`); `20260918000000_market_prices_tips_entitlement_rls.sql` (closes the anon read hole). Deployment runbook: `docs/EDGE_FUNCTIONS_RUNBOOK.md`
- [ ] Register the Telegram webhook once deployed: `https://<ref>.supabase.co/functions/v1/telegram-webhook` (setWebhook with `secret_token` = `TELEGRAM_WEBHOOK_SECRET`), and the weekly freshness_cron trigger (Mon 06:00 UTC) in the Supabase dashboard scheduled-function settings
