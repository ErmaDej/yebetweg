...
4. **Post-launch** (optional): chart lib consolidation, analytics dashboard, expand test coverage to hooks/pages. Phase 5 progress: freshness cron automation ✓, Telegram /submitprice funnel ✓, multi-city BOQ from market_prices ✓, Pro export ✓ + BOQ→actuals ✓ (Sep 21), assistant intelligence upgrade ✓ (Sep 21), **save-to-project polish ✓ (Sep 21: `/boq/:token` share permalinks + saved collections)**. Phase 4 a11y closed Sep 21 (chips, live regions, mobile tab labels; floating-labels item audited stale). Owner still must apply migrations `20260921000000` (boq_actuals), `20260921010000` (submit_rfq anon fix), `20260921020000` (share tokens). Reference docs: `docs/PROJECT_BIBLE.md`, `docs/VERCEL_LAUNCH_CHECKLIST.md`.
